const API='https://gregarious-frangollo-24145c.netlify.app/api/baas';
const native=async(path,options={})=>{if(typeof window!=='undefined'&&typeof window.__rfNativeApi==='function'){return window.__rfNativeApi({method:options.method||'GET',path,body:options.body||null,token:localStorage.getItem('rf-token')})}return null};
const cacheBust=(path,method='GET')=>method==='GET'?(path.includes('?')?`${path}&_=${Date.now()}`:`${path}?_=${Date.now()}`):path;
const normalize=(v)=>{if(Array.isArray(v))return v.map(normalize);if(v&&typeof v==='object'){if(v.type==='null')return null;if(Object.prototype.hasOwnProperty.call(v,'type')&&Object.prototype.hasOwnProperty.call(v,'value')&&Object.keys(v).length<=2)return normalize(v.value);const o={};for(const k of Object.keys(v))o[k]=normalize(v[k]);return o}return v};
const clearSession=()=>{try{localStorage.removeItem('rf-token');localStorage.removeItem('rf-user')}finally{if(typeof window!=='undefined')window.dispatchEvent(new CustomEvent('rf-auth-expired'))}};
export async function apiFetch(path,options={}){try{const n=await native(path,options);if(n!==null)return normalize(n)}catch(e){throw e instanceof Error?e:new Error(String(e))}const method=(options.method||'GET').toUpperCase(),h={Accept:'application/json','Content-Type':'application/json','Cache-Control':'no-cache',...(options.headers||{})};const t=localStorage.getItem('rf-token');if(t)h.Authorization=`Bearer ${t}`;let r;try{r=await fetch(API+cacheBust(path,method),{...options,method,headers:h,cache:'no-store'})}catch(e){throw new Error('Не вдалося підключитися до RehaFlow API')}const text=await r.text();let d=null;try{d=text?JSON.parse(text):null}catch{}if(r.status===401&&!path.startsWith('/auth/login')){clearSession();throw new Error('Сесія завершилась. Увійдіть знову.')}if(!r.ok)throw new Error(d?.error?.message||d?.error||d?.message||text||`HTTP ${r.status}`);return normalize(d)}
export async function login(identifier,password){const d=await apiFetch('/auth/login',{method:'POST',body:JSON.stringify({email:identifier,password,client:'desktop',deviceId:'windows-desktop',deviceName:navigator?.userAgent?.includes('Windows')?'RehaFlow Windows':'RehaFlow Desktop',platform:'windows',appVersion:'0.20.0'})});const t=d?.accessToken||d?.token;if(t)localStorage.setItem('rf-token',t);if(d?.user)localStorage.setItem('rf-user',JSON.stringify(d.user));return d}
export const logout=async()=>{try{await apiFetch('/auth/logout',{method:'POST',body:'{}'})}catch{}finally{clearSession()}};
export const me=()=>apiFetch('/auth/me');
export const getDashboard=async()=>{const d=await apiFetch('/dashboard');const s={...(d?.stats||{})};if(s.totalBeds===undefined){const b=await apiFetch('/beds');const rows=b?.beds||[];s.totalBeds=rows.length;s.occupiedBeds=rows.filter(x=>x.status==='occupied').length;s.availableBeds=rows.filter(x=>x.status==='available').length;s.cleaningBeds=rows.filter(x=>x.status==='cleaning').length;s.maintenanceBeds=rows.filter(x=>x.status==='maintenance').length}return {...d,stats:s,updatedAt:d?.updatedAt||new Date().toISOString()}};
export const getPatients=(q='')=>apiFetch(`/patients?status=active&limit=2000${q?`&q=${encodeURIComponent(q)}`:''}`);
export const getPatient=id=>apiFetch(`/patients/${id}`);
export const createPatient=b=>apiFetch('/patients',{method:'POST',body:JSON.stringify(b)});
export const updatePatient=(id,b)=>apiFetch(`/patients/${id}`,{method:'PATCH',body:JSON.stringify(b)});
export const archivePatient=id=>apiFetch(`/patients/${id}/archive`,{method:'POST',body:'{}'});
export const getArchive=()=>apiFetch('/archive');
export const getPatientHistory=()=>apiFetch('/patient-history');
export const getRooms=()=>apiFetch('/rooms');
export const createRoom=b=>apiFetch('/rooms',{method:'POST',body:JSON.stringify(b)});
export const getBeds=()=>apiFetch('/beds');
export const createBed=b=>apiFetch('/beds',{method:'POST',body:JSON.stringify(b)});
export const assignBed=(id,patientId)=>apiFetch(`/beds/${id}`,{method:'PATCH',body:JSON.stringify({patientId})});
export const releaseBed=id=>apiFetch(`/beds/${id}/release`,{method:'POST',body:'{}'});
export const getTasks=()=>apiFetch('/tasks');
export const createTask=b=>apiFetch('/tasks',{method:'POST',body:JSON.stringify(b)});
export const updateTask=(id,b)=>apiFetch(`/tasks/${id}`,{method:'PATCH',body:JSON.stringify(b)});
export const deleteTask=id=>apiFetch(`/tasks/${id}`,{method:'DELETE'});
export const getPrescriptions=()=>apiFetch('/prescriptions');
export const createPrescription=b=>apiFetch('/prescriptions',{method:'POST',body:JSON.stringify(b)});
export const getDocuments=(patientId='')=>apiFetch(`/documents${patientId?`?patientId=${encodeURIComponent(patientId)}`:''}`);
export const createDocument=b=>apiFetch('/documents',{method:'POST',body:JSON.stringify(b)});
export const getUsers=(q='')=>apiFetch(`/users${q?`?q=${encodeURIComponent(q)}`:''}`);
export const createUser=b=>apiFetch('/users',{method:'POST',body:JSON.stringify(b)});
export const updateUser=(id,b)=>apiFetch(`/users/${id}`,{method:'PATCH',body:JSON.stringify(b)});
export const deleteUser=id=>apiFetch(`/users/${id}`,{method:'DELETE'});
export const getRoles=()=>apiFetch('/roles');
export const getSessions=()=>apiFetch('/sessions');
export const revokeSession=id=>apiFetch(`/sessions/${id}`,{method:'DELETE'});
export const getAudit=()=>apiFetch('/audit');
export const getDoctors=()=>apiFetch('/staff/doctors');
export const getTreatmentRecords=(patientId='')=>apiFetch(`/treatment-records${patientId?`?patientId=${encodeURIComponent(patientId)}`:''}`);
export const createTreatmentRecord=b=>apiFetch('/treatment-records',{method:'POST',body:JSON.stringify(b)});
export const updateTreatmentRecord=(id,b)=>apiFetch(`/treatment-records/${id}`,{method:'PATCH',body:JSON.stringify(b)});
export const getAppointments=()=>apiFetch('/appointments');
export const createAppointment=b=>apiFetch('/appointments',{method:'POST',body:JSON.stringify(b)});
